...
const InputField2 = ({ label, id, inputClasses, containerClasses, type, placeholder, role, value, name, defaultChecked, disabled, onChange, note, myref, autoComplete, ariaDescribedby, ariaInvalid, onFocus, onBlur, required, showNote }) => {
			...
			<div className={` ${containerClasses ? containerClasses : ' '} ${isRadio || disabled ? ' flex items-center  ' : ' flex flex-col '} ${disabled ? " gap-1" : ""} `}>
				...
				>
					...
					<input
						...
						className={`block w-full px-9 py-2 rounded-md shadow-sm placeholder-slate-400  placeholder:italic 
